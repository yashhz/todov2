    ...
    return (
        <div className={`smart-input-container ${expanded ? 'smart-input--expanded' : ''}`} ref={containerRef}>
            <div className="smart-input-main">
                ...
                />
                {input.trim() && (
                     <button className="smart-input-submit" onClick={() => { onSubmit(parsedBatch); setInput(''); setDismissed(new Set()); setExpanded(false); }} title="Submit" aria-label="Submit command">
                         ↑
                     </button>
                )}
            </div>
                ...
                <div className="smart-input-chips">
                    {parsed.tokens.map((token, i) => (
                        <span key={i} className="smart-chip" style={{ '--chip-color': TOKEN_COLORS[token.type] } as React.CSSProperties}>
                            ...
                            <button className="smart-chip-dismiss" onClick={(e) => { e.stopPropagation(); dismissToken(token.raw); }} title="Remove token" aria-label="Remove token">×</button>
                        </span>
                    ))}
                </div>
            )}
