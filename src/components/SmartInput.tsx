    ...
    return (
        <div className={`smart-input-container ${expanded ? 'smart-input--expanded' : ''}`} ref={containerRef}>
            <div className="smart-input-main">
                ...
                />
                {input.trim() && (
                     <button className="smart-input-submit" onClick={() => { onSubmit(parsedBatch); setInput(''); setDismissed(new Set()); setExpanded(false); }} title="Submit command" aria-label="Submit command">
                         ↑
                     </button>
                )}
            </div>
