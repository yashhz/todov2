    ...
    return (
        <div className={`smart-input-container ${expanded ? 'smart-input--expanded' : ''}`} ref={containerRef}>
            <div className="smart-input-main">
                ...
                />
                {input.trim() && (
                     <button type="button" className="smart-input-submit" aria-label="Submit command" title="Submit command" onClick={() => { onSubmit(parsedBatch); setInput(''); setDismissed(new Set()); setExpanded(false); }}>
                         ↑
                     </button>
                )}
            </div>
                ...
                <div className="smart-input-chips">
                    {parsed.tokens.map((token, i) => (
                        <span key={i} className="smart-chip" style={{ '--chip-color': TOKEN_COLORS[token.type] } as React.CSSProperties}>
                            ...
                            <button type="button" className="smart-chip-dismiss" aria-label={`Dismiss ${tokenLabel(token, goals, projects)}`} title="Dismiss token" onClick={(e) => { e.stopPropagation(); dismissToken(token.raw); }}>×</button>
                        </span>
                    ))}
                </div>
            )}
            ...
            {expanded && (
                <div className="smart-input-drawer animate-fade-in-up">
                    <div className="smart-drawer-toolbar">
                        <button type="button" className="smart-toolbar-btn" title="Set Date" aria-label="Set Date"><Calendar size={13} /> {parsed?.date ? formatParsedDate(parsed.date) : 'Date'}</button>
                        <button type="button" className="smart-toolbar-btn" title="Set Time" aria-label="Set Time"><Clock size={13} /> {parsed?.time ? formatParsedTime(parsed.time) : 'Time'}</button>
                        <button type="button" className="smart-toolbar-btn" title="Set Priority" aria-label="Set Priority"><Flag size={13} /> {parsed?.priority ? parsed.priority : 'Priority'}</button>
                        <button type="button" className="smart-toolbar-btn" title="Link Goal" aria-label="Link Goal"><Target size={13} /> Goal</button>
                        <button type="button" className="smart-toolbar-btn" title="Add Tag" aria-label="Add Tag"><Tag size={13} /> Tag</button>
                    </div>
