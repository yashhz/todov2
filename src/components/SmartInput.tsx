                ...
                <div className="smart-input-chips">
                    {parsed.tokens.map((token, i) => (
                        <span key={i} className="smart-chip" style={{ '--chip-color': TOKEN_COLORS[token.type] } as React.CSSProperties}>
                            ...
                            <button type="button" className="smart-chip-dismiss" onClick={(e) => { e.stopPropagation(); dismissToken(token.raw); }} aria-label="Dismiss token" title="Dismiss token">×</button>
                        </span>
                    ))}
                </div>
            )}
