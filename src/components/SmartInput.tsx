    ...
    return (
        <div className={`smart-input-container ${expanded ? 'smart-input--expanded' : ''}`} ref={containerRef}>
            <div className="smart-input-main">
                ...
                />
                {input.trim() && (
                     <button type="button" className="smart-input-submit" aria-label="Submit command" title="Submit" onClick={() => { onSubmit(parsedBatch); setInput(''); setDismissed(new Set()); setExpanded(false); }}>
                         ↑
                     </button>
                )}
            </div>
                ...
                <div className="smart-input-chips">
                    {parsed.tokens.map((token, i) => (
                        <span key={i} className="smart-chip" style={{ '--chip-color': TOKEN_COLORS[token.type] } as React.CSSProperties}>
                            ...
                            <button type="button" className="smart-chip-dismiss" aria-label="Remove token" title="Remove token" onClick={(e) => { e.stopPropagation(); dismissToken(token.raw); }}>×</button>
                        </span>
                    ))}
                </div>
            )}
