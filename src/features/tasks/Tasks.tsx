                    ...
                    <div className={`m-form__body ${formTitle.trim() ? 'm-form__body--visible' : ''}`}>
                        ...
                        <div className="m-form__section">
                            <div className="m-form__row">
                                ...
                                {formDate && (
                                    <button type="button" className="m-form__clear-date" onClick={() => { setFormDate(''); setShowCalendar(false); }} style={{background:'transparent', border:'none', color:'rgba(255,255,255,0.25)', cursor:'pointer'}} title="Clear date" aria-label="Clear date">×</button>
                                )}
                            </div>
                        ...
                        <div className="m-form__section">
                            <div className="m-form__section-row-head" style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                                ...
                                {formTime && (
                                    <button type="button" className="m-form__time-clear" onClick={() => setFormTime('')} style={{background:'transparent', border:'none', fontSize:'10px', color:'rgba(255,255,255,0.25)', textTransform:'uppercase', cursor:'pointer'}} title="Clear time" aria-label="Clear time">
                                        clear
                                    </button>
                                )}
                            </div>
                                ...
                                {formProjectId && (() => {
                                    ...
                                    return (
                                        <span className="m-form__pill m-form__pill--sm" style={{ background: p.color + '18', borderColor: p.color + '55', color: p.color }}>
                                            {p.icon} {p.name}
                                            <button type="button" onClick={() => setFormProjectId(null)} style={{background:'transparent', border:'none', color:'inherit', marginLeft:'6px', cursor:'pointer'}} title="Remove project" aria-label="Remove project">×</button>
                                        </span>
                                    );
                                })()}
                                ...
                                {formGoalLinks.map(link => {
                                    ...
                                    return (
                                        <span key={link.goalId} className="m-form__pill m-form__pill--sm" style={{color:'var(--text-secondary)'}}>
                                            {g.icon} {g.title}
                                            <button type="button" onClick={() => setFormGoalLinks(formGoalLinks.filter(l => l.goalId !== link.goalId))} style={{background:'transparent', border:'none', color:'inherit', marginLeft:'6px', cursor:'pointer'}} title="Remove goal" aria-label="Remove goal">×</button>
                                        </span>
                                    );
                                })}
