                    ...
                    <div className={`m-form__body ${formTitle.trim() ? 'm-form__body--visible' : ''}`}>
                        ...
                        <div className="m-form__section">
                            <div className="m-form__row">
                                ...
                                {formDate && (
                                    <button type="button" className="m-form__clear-date" aria-label="Clear date" title="Clear date" onClick={() => { setFormDate(''); setShowCalendar(false); }} style={{background:'transparent', border:'none', color:'rgba(255,255,255,0.25)', cursor:'pointer'}}>×</button>
                                )}
                            </div>
                        ...
                        <div className="m-form__section">
                            <div className="m-form__section-row-head" style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                                ...
                                {formTime && (
                                    <button type="button" className="m-form__time-clear" aria-label="Clear time" title="Clear time" onClick={() => setFormTime('')} style={{background:'transparent', border:'none', fontSize:'10px', color:'rgba(255,255,255,0.25)', textTransform:'uppercase', cursor:'pointer'}}>
                                        clear
                                    </button>
                                )}
                            </div>
                                ...
                                {formProjectId && (() => {
                                    ...
                                    return (
                                        <span className="m-form__pill m-form__pill--sm" style={{ background: p.color + '18', borderColor: p.color + '55', color: p.color }}>
                                            {p.icon} {p.name}
                                            <button type="button" aria-label="Remove project" title="Remove project" onClick={() => setFormProjectId(null)} style={{background:'transparent', border:'none', color:'inherit', marginLeft:'6px', cursor:'pointer'}}>×</button>
                                        </span>
                                    );
                                })()}
                                ...
                                {formGoalLinks.map(link => {
                                    ...
                                    return (
                                        <span key={link.goalId} className="m-form__pill m-form__pill--sm" style={{color:'var(--text-secondary)'}}>
                                            {g.icon} {g.title}
                                            <button type="button" aria-label="Remove goal" title="Remove goal" onClick={() => setFormGoalLinks(formGoalLinks.filter(l => l.goalId !== link.goalId))} style={{background:'transparent', border:'none', color:'inherit', marginLeft:'6px', cursor:'pointer'}}>×</button>
                                        </span>
                                    );
                                })}
