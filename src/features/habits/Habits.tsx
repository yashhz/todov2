...
function DateNavigator({ selectedDate, onDateChange, habits }: { selectedDate: Date; onDateChange: (d: Date) => void; habits: Habit[] }) {
    ...
    return (
        <div className="date-navigator">
            <button type="button" className="dn-arrow" onClick={() => {
                const prev = new Date(selectedDate);
                prev.setDate(selectedDate.getDate() - 1);
                onDateChange(prev);
            }} title="Previous day" aria-label="Previous day"><ChevronLeft size={16} /></button>
            ...
            <button type="button" className="dn-arrow" onClick={() => {
                ...
                onDateChange(next);
            }} title="Next day" aria-label="Next day"><ChevronRight size={16} /></button>
        </div>
    );
}
                            ...
                            <div style={{ position: 'relative', flex: 1, minWidth: '150px' }}>
                                {formProjectId ? (
                                    <div style={{ display: 'flex', alignItems: 'center' }}>
                                        ...
                                        <button type="button" onClick={() => setFormProjectId(null)} style={{ background: 'none', border: 'none', color: 'var(--text-tertiary)', cursor: 'pointer', marginLeft: '8px' }} title="Remove project" aria-label="Remove project">×</button>
                                    </div>
                                ) : (
                                    <>
                                        <input
