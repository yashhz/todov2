...
function DateNavigator({ selectedDate, onDateChange, habits }: { selectedDate: Date; onDateChange: (d: Date) => void; habits: Habit[] }) {
    ...
    return (
        <div className="date-navigator">
            <button className="dn-arrow" aria-label="Previous day" title="Previous day" onClick={() => {
            ...
            <button className="dn-arrow" aria-label="Next day" title="Next day" onClick={() => {
                    ...
                    <div className={`m-form__body ${formTitle.trim() ? 'm-form__body--visible' : ''}`}>
                        ...
                        <div className="m-form__section m-form__section--row">
                            ...
                            <div style={{ position: 'relative' }}>
                                ...
                                {formTime && (
                                    <button type="button" aria-label="Clear time" title="Clear time" onClick={() => setFormTime('')} style={{background:'transparent', border:'none', color:'rgba(255,255,255,0.25)', cursor:'pointer', marginLeft: '8px'}}>×</button>
                                )}
                                {showTimePicker && (
                            ...
                            <div style={{ position: 'relative', flex: 1, minWidth: '150px' }}>
                                {formProjectId ? (
                                    <div style={{ display: 'flex', alignItems: 'center' }}>
                                        ...
                                        <button type="button" aria-label="Clear project" title="Clear project" onClick={() => setFormProjectId(null)} style={{ background: 'none', border: 'none', color: 'var(--text-tertiary)', cursor: 'pointer', marginLeft: '8px' }}>×</button>
                                    </div>
                                ) : (
                                    <>
                                        <input
